export const formatDatesToTimeRange = (
  start: Date,
  end: Date,
  ...
  if (dayStart === dayEnd && monthStart === monthEnd && yearStart === yearEnd) {
    ...
    return (
      `${dayStart}.${monthStart}.${yearStart} ${hoursStart}:${minutesStart} ` +
      `- ${dayEnd}.${monthEnd}.${yearEnd} ${hoursEnd}:${minutesEnd}`
    );
  }
};

const getDateTimeVars = (
  date: Date,
  timeZone: string,
): {
  day: string;
  ...
  const formatted = formatter.formatToParts(date).reduce(
    (acc, part) => {
